...
# ========== 性价比比较模式 ==========
if mode == "性价比比较":
    st.info("计算提升同样属性花费多少铜扣才等于花费一把刀（新应用上线，有问题请向1128-玩大蛋反馈）")
    st.markdown("---")
    ...
    with st.expander("点击展开，比较琢玉刀与铜扣的性价比", expanded=True):
        ...
        with col_b:
            horse_buckle_cost = st.number_input(
                ...
                value=1, 
                step=1,
                help="每次升级马具需要消耗的铜扣个数（程序会自动按4倍计算）"
            )
            horse_attr_percent = st.number_input(
                "升级马具提升的属性百分比 (%)(马具颜色不同升级加的百分比不同)", 
                min_value=0.0, 
                value=0.25, 
                step=0.1,
                format="%.2f",
        ...
        if st.button("计算性价比", key="ratio_calc"):
            actual_buckle_cost = horse_buckle_cost * 4
            ...
            if horse_attr_percent * conversion_factor == 0:
                ...
                with st.expander("查看计算过程"):
                    ...
                    - 性价比比值 = (每琢玉刀属性) / (每铜扣属性) = **{knife_to_buckle:.4f}**  
                    - 结论：1个琢玉刀 ≈ **{knife_to_buckle:.2f}** 个铜扣
                    """)
    
    st.caption("提示：铜扣消耗已按4倍计算（游戏实际消耗）。")
    st.stop()  # 结束页面，不显示后续神兵玉石升级内容
...
with st.sidebar:
    ...
    st.subheader("兑换比例")
    POINTS_PER_WOOD = st.number_input("木头兑换比例", min_value=0.0, value=0.25, step=0.1, format="%.3f")
    POINTS_PER_MITHRIL = st.number_input("精金兑换比例", min_value=0.0, value=5.0, step=0.1, format="%.2f")
    POINTS_PER_LAPIS = st.number_input("青金石兑换比例", min_value=0.0, value=15.0, step=0.1, format="%.2f")
    POINTS_PER_CARVING_KNIFE = st.number_input("琢玉刀兑换比例", min_value=0.0, value=75.0, step=1.0, format="%.2f")
    POINTS_PER_UNPOLISHED_JADE = st.number_input("璞玉兑换比例", min_value=0.0, value=15.0, step=0.1, format="%.2f")
...
for i in range(st.session_state.other_rows):
    cols = st.columns([3, 1, 1])
    with cols[0]:
        st.text_input("", key=f"other_name_{i}", placeholder="物品名称", label_visibility="collapsed")
    with cols[1]:
        st.number_input("", key=f"other_cost_{i}", min_value=0.0, value=0.0, step=1.0, format="%.1f", label_visibility="collapsed")
    with cols[2]:
...
if st.button("开始自动计算最佳升级方案", type="primary", use_container_width=True):
    ...
    for i in range(st.session_state.other_rows):
        ...
        times = st.session_state.get(f"other_times_{i}", 1)
        other_total += cost * times

    if other_total > CURRENT_POINTS:
        st.error(f"❌ 其他兑换所需积分 {other_total:.1f} 超过当前总积分 {CURRENT_POINTS}，无法进行神兵玉石升级。请减少其他兑换或增加积分。")
    else:
        ...
        if not result["upgraded"]:
            st.warning("当前剩余积分和材料无法进行任何神兵玉石升级！请检查您的资源或降低等级差设置。")
            ...
            with points_col2:
                st.metric("其他兑换消耗", f"{other_total:.1f}")
            with points_col3:
                st.metric("神兵玉石所需", f"{result['points_needed']:.1f}")
            with points_col4:
...
st.markdown("---")
st.caption("提示：修改侧边栏的设置后，点击上方按钮重新计算。切换模式后，当前设置会被重置。")
