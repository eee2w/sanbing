...
with st.expander("点击展开，比较琢玉刀与铜扣的性价比", expanded=True):
    ...
    if st.button("计算性价比", key="ratio_calc"):
        ...
        # 玉石每琢玉刀带来的属性提升 = (jade_attr_percent) / jade_knife_cost
        # 马具每铜扣带来的属性提升 = (horse_attr_percent * conversion_factor) / actual_buckle_cost
        # 一个琢玉刀相当于的铜扣数 = (每琢玉刀属性) / (每铜扣属性) 
        # = (jade_attr_percent / jade_knife_cost) / (horse_attr_percent * conversion_factor / actual_buckle_cost)
        # = (jade_attr_percent * actual_buckle_cost) / (jade_knife_cost * horse_attr_percent * conversion_factor)
